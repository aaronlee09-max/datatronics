(() => {
  ...
  async function unlock(account) { document.body.classList.remove("auth-locked"); document.querySelector("#fontoryAuth")?.remove(); document.querySelector("#fontoryPasskeyPanel")?.remove(); document.querySelector("#fontoryEmailPanel")?.remove(); document.querySelector(".auth-logout")?.remove(); document.querySelector(".auth-passkey-manage")?.remove(); document.querySelector(".auth-email-manage")?.remove(); document.querySelector(".auth-daily-code")?.remove(); document.querySelector(".auth-admin-manage")?.remove(); document.querySelector("#fontoryAdminPanel")?.remove(); const topbar = document.querySelector(".topbar"); if (!topbar) return; if (account.method === "server") { const emailBtn = document.createElement("button"); emailBtn.type = "button"; emailBtn.className = "auth-email-manage"; emailBtn.textContent = "이메일 설정"; emailBtn.addEventListener("click", () => showEmailPanel(account)); topbar.appendChild(emailBtn); } if (account.role === "admin") { const adminBtn = document.createElement("button"); adminBtn.type = "button"; adminBtn.className = "auth-admin-manage"; adminBtn.textContent = "계정 관리"; adminBtn.addEventListener("click", () => showAdminPanel(account)); topbar.appendChild(adminBtn); } const passkeyBtn = document.createElement("button"); passkeyBtn.type = "button"; passkeyBtn.className = "auth-passkey-manage"; passkeyBtn.textContent = "패스키 만들기"; passkeyBtn.addEventListener("click", () => showPasskeyPanel(account)); topbar.appendChild(passkeyBtn); const logout = document.createElement("button"); logout.type = "button"; logout.className = "auth-logout"; logout.textContent = account.role === "admin" ? "관리자 · 로그아웃" : "로그아웃"; logout.addEventListener("click", async () => { if (account.method === "server") await centralLogout(); sessionStorage.removeItem(KEY); sessionStorage.removeItem(PENDING_KEY); location.reload(); }); topbar.appendChild(logout); }
